...
#combining different types lead to less restrictive type 
c(FALSE,3) 
c(pi, 'abc') 
c(FALSE, 'abc') 
#sequence 
...
#transpose 
t(x) 
#lower case letters, month names and month abbreviations - useful vectors 
letters[1:3] 
month.name[1:3] 
month.abb[1:3] 
#combining vectors 
...
#1.2.8. FACTORS 
pain=c(0,3,2,2,1) #even though 
fpain=factor(pain,levels=0:3)#even though levels are form 0 to 3, internal representation is 1 to 4 - that's what as.numeric will return 
...
#the same can be acheived using split function: 
l=split(energy$expend, energy$stature) 
#1.2.16 IMPLICIT LOOPS 
apply(thuesen, mean, na.rm=T) 
apply(thuesen, mean, na.rm=T) 
...
save.image() 
#default name is .RData - will be loaded y default when R is started 
#2.1.2 Textual output 
#redirect output to myfile 
sink( "myfile") 
ls() 
#restore console output 
sink()
#another way to capture the console output 
...
#history of commands can be saved/loaded using 
...
data(thuesen) 
#data goes through directories assiciated twith each package and looks for the data 
#files with .R exception are executed, .tab files read using read. table 
#2.1.7 attach and detach 
#long and heavy 
plot(thuesen$blood.glucose, thuesen$short.velocity) 
#attach allows R look for objects among the vars in a given dataframe 
attach(thuesen) 
blood.glucose 
search()#see thuesen dataframe among the objects in the search path 
#.GlobalEnv has a priority, but we can explicitly specify that we need use the dataset 
with(thuesen, plot(blood.glucose, short.velocity))
#detach with no arguments remove the object in position 2. 
detach() 
search() 
...
#to plot margins, there is a mtext function- can plot on 4 margins 
for(side in 1:4) mtext(-1:4, side=side, at=.7, line=-1:4) 
text(paste('side' ,1:4),side=1:4,line=0,font=2)#line parameter controls relative position to the chart box 
#font=2- Bold 
...
#2.2.3 Using par 
par(mar=c(4,4,2,2)+0.1)#reducing margins - dfaults are 5,4,4,2 
...
#slightly better- top of density is not clipped, but the right tail still is 
curve(dnorm(x)) 
hist(x,freq=F, add=T) 
#better 
h=hist(x, plot=F) 
...
secretin2=read.table(tmpPath, header=T) 
secretin2 
class(secretin2) 
levels (secretin2$time) 
#2.4.2 File format details (read.table)_ 

#header=T 
#Field separator: sep=',' 
#NA strings: na.strings=' .' 
#quotes and comments: defaul t - "#". quote="'" comment. char=' ! ' 
#Unequal field count: normally - error, fill and flush arguents are used to coorect 
#pre-cooked variants: read.csv (separated by commas) and read.csv2 (separated by;), header=T 
...
dd=data.frame()
fix(dd) 
#2.4.4 interfacing to other prograns 
